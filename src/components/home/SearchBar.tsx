...
const SearchItems: string[] = ['Search "Burger"', 'Search "Pizza"', 'Search "Pasta"', 'Search "Biriyani"']
const SearchBar = () => {
  ...
  const textColorAnimation=useAnimatedStyle(()=>{
    const textColor=interpolate(scrollGlobal.value,[0,80],[255,0])
    return{
...
<View style={[styles.flexRowBetween,styles.padding]}>
  ...
  {SearchItems?.map((item,index)=>{ 
    return(
      <CustomText fontSize={12} fontFamily='Okra-Medium' key={index} style={styles.rollingText} >{item}</CustomText>
    )
  }
  )}
  </RollingContent>
  ...
  <Pressable style={styles.vegMode} onPress={()=>dispatch(setVegMode(!isVegMode))}>
     ...
     <Image source={isVegMode ? require('@assets/icons/switch_on.png'):
      require('@assets/icons/switch_off.png')} style={styles.switch
     } />
  </Pressable>
